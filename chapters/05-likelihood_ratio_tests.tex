...
\subsubsection{Application of T-test}  \label{ex:ttest}
...
\subsubsection{Application of \textit{F}-test}
...
\subsection{Application of K-fold Cross-Validation}
...
In the case of Copenhagen we chose 5 parameters as this has the lowest prediction error. 
...
% Fra White: 
% When heteroskedastiticy is present linear models are still consistent however their parameter estimates are inefficent and their covariance matrix estimates are inconsistent, i.e. \Var(\betahat | \mathbf{X}). This leads to wrongfull conclusions when hypotheses tests as t/- and F-tests are used on the model, when heteroskedasticity is present. 

% The inefficiency in the parameter estimates is caused by the inconsistency of the variance matrix of the OLS parameter estimates. For this reason we wish to introduce covariance matrix estimates, i.e. \Var(\betahat | \mathbf{X}), which are consistent when heteroskedasticity is present. 

% Recall our model given as
% \begin{align*}
% y_i = \mathbf{X}_\betahat_0 + \varepsilon_i. 
% \end{align*}
% Where $(\mathbf{X}_i, \varepsilon_i)$ are \textbf{independent but not necessarily idetically distributed (i.n.i.d)} for observation $i$. This assumption means that we assume only $\mathbf{X}_i$ and $\varepsilon_i$ are uncorrelated, so the Gauss Markov assumption stating $E[\varepsilon|\mathbf{X}_i] = 0$ holds true and heteroskedasticity of the form $E[\varepsilon__i^2] = g(\mathbf{X}_i)$, where $g$ is a known function.

% We state the following assumption

% \begin{Assumption}[]
% \begin{itemize}
% \item There exist positive finite constants $\delta$ and $\Delta$ such that $E[ |\varepsilon^2_i |^{1 + \delta} ] < \Delta$ and $E[ |\mathbf{X}_{ij}\mathbf{X}_ik|^{1 + \delta}] < \Delta$ for all $i$ and $j,k = 1, \ldots, K$\\
% \item $\overline{M}_n = n^{-1}\nsum E[\mathbf{X}^{\top}_i \mathbf{X}_i]$ is nonsingular for $n$ sufficiently large, such that $\overline{M}_n > \delta > 0$
% \end{itemize}
% \end{Assumption}

% The first part of the assumption ensures that all the error variances and the variances of $\betahat$ are bounded by the same constant, i.e. they are uniformly bounded. 
% The second part ensures that the elements of inverse are uniformly bounded and that the average $\Var(\betahat | \mathbf{X})$ will for sufficiently large $n$ be nonsingular. 



If our model contains heteroskedasticity, then the error will not be constant. Thus for our model, $\mathbf{y}_i = \betabold \mathbf{X} + \varepsilon_i$, the variance of the error term is given as
\begin{align*}
...
For an example consider the general linear model $\mathbf{y} = \betabold_0 + \betabold_{i,1} + \betabold_{i,2} + \betabold_{i,3} + \varepsilon_i$. We wish to test if $\betabold_2 = \betabold_3$ and if $\betabold_3 = 0$. These two equations are the hypothesis and thus $q = 2$. We represent in matrix form as
\begin{align*}
